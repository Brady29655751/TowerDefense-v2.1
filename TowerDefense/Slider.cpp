...
void Slider::OnMouseDown(int button, int mx, int my) {
	...
	if ((button & 1) && ImageButton::mouseIn) {
		Down = true;
	}
}
...
void Slider::OnMouseMove(int mx, int my) {
	...
	ImageButton::OnMouseMove(mx, my);   // Update MouseIn.
	if (Down && mx <= End2.Position.x && mx >= End1.Position.x) {
		float new_value = (mx - End1.Position.x) / (End2.Position.x - End1.Position.x);
		SetValue(new_value);
	}
}
